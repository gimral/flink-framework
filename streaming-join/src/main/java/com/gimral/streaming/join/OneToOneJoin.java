...
public class OneToOneJoin<K, L, R>
  ...
  public void innerOpen(OpenContext ctx) {
    ...
    rightRecordState =
        runtimeCtx.getState(
            new ValueStateDescriptor<>(RIGHT_RECORD, TypeInformation.of(new TypeHint<>() {})));
  ...
  public void innerProcessElement1(L input, Context ctx, Collector<Tuple2<K, Tuple2<L, R>>> out) {
    ...
    if (expireTimestamp != null) ctx.timerService().deleteProcessingTimeTimer(expireTimestamp);
    if (rightValue != null) {
